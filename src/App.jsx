/**
 * @fileoverview Portal de Gestión ISO 9001 + OEA - Aplicación React completa
...
function App() {
  ...
  const [config, setConfig] = useLocalStorage("portal_config", {
    ...
    modulos: {
      ...
      iso_demo: { activo: true, nombre: "Políticas ISO 9001 Demo", emoji: "🎯" },
  ...
  // Asegurar que el módulo iso_demo siempre esté presente
  useEffect(() => {
    if (config.modulos && !config.modulos.iso_demo) {
      setConfig({
        ...config,
        modulos: {
          ...config.modulos,
  ...
  const [docsISODemo, setDocsISODemo] = useLocalStorage("portal_docs_iso_demo", []);
  ...
  const [tabActivoConfig, setTabActivoConfig] = useState("empresa");
  ...
  useEffect(() => {
    ...
    if (docsISODemo.length === 0 && localStorage.getItem("portal_seeded")) {
      const demoData = JSON.parse(localStorage.getItem("portal_docs_iso_demo") || "[]");
      if (demoData.length === 0) {
        // Si no hay datos demo, cargar solo los datos demo
        const now = new Date().toISOString();
        const isoDemoDocs = [
          {
            id: generateId(),
            codigo: "POL-CAL-DEMO-001",
            titulo: "Política de Calidad Integral",
            descripcion: "Política general que establece el compromiso de la organización con la calidad total, la mejora continua y la excelencia en el servicio.",
            version: "v2.1",
            estado: "Vigente",
            categoria: "Calidad",
            responsable: "Dirección General",
            fechaCreacion: new Date(Date.now() - 180 * 86400000).toISOString(),
            fechaActualizacion: new Date(Date.now() - 15 * 86400000).toISOString(),
            fechaVigencia: new Date(Date.now() + 45 * 86400000).toISOString(),
            etiquetas: ["calidad", "iso", "política"],
            subidoPor: "Admin",
            archivo: null,
            historial: [{ version: "v2.1", fecha: new Date(Date.now() - 15 * 86400000).toISOString(), autor: "Admin", nota: "Versión actualizada" }]
          }
        ];
        setDocsISODemo(isoDemoDocs);
        localStorage.setItem("portal_docs_iso_demo", JSON.stringify(isoDemoDocs));
      }
    }
  }, []);
  ...
  const getDocsActuales = () => {
    switch (seccion) {
      ...
      case "iso_demo": return docsISODemo;
  ...
  const setDocsActuales = (docs) => {
    switch (seccion) {
      ...
      case "iso_demo": setDocsISODemo(docs); break;
  ...
  const documentosFiltrados = useMemo(() => {
    ...
    return docs;
  }, [seccion, busquedaLocal, filtroEstado, filtroCategoria, ordenarPor, docsISO, docsISODemo, docsOEA, docsProcedimientos, docsFormatos, docsManuales, docsComunicados]);
  ...
  const estadisticas = useMemo(() => {
    const modulos = [
      { key: "iso", docs: docsISO, emoji: config.modulos?.iso?.emoji || "📋", nombre: config.modulos?.iso?.nombre || "Políticas ISO 9001" },
      { key: "iso_demo", docs: docsISODemo, emoji: config.modulos?.iso_demo?.emoji || "🎯", nombre: config.modulos?.iso_demo?.nombre || "Políticas ISO 9001 Demo" },
      { key: "oea", docs: docsOEA, emoji: config.modulos?.oea?.emoji || "🛡️", nombre: config.modulos?.oea?.nombre || "Requisitos OEA" },
  ...
  }, [docsISO, docsISODemo, docsOEA, docsProcedimientos, docsFormatos, docsManuales, docsComunicados, config]);

  // Documentos próximos a vencer
  const proximosVencer = useMemo(() => {
    const todos = [...docsISO, ...docsISODemo, ...docsOEA, ...docsProcedimientos, ...docsFormatos, ...docsManuales, ...docsComunicados];
    return todos
    ...
    return (
      <div className="sidebar" style={{
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 0" }}>
          ...
          {/* Módulo Demo - siempre visible */}
          <div
            onClick={() => { setSeccion("iso_demo"); setPaginaActual(1); }}
            className="nav-item"
            style={{
              padding: sidebarCollapsed ? "16px" : "12px 24px",
              cursor: "pointer",
              backgroundColor: seccion === "iso_demo" ? `${config.colorAcento}33` : "transparent",
              borderLeft: seccion === "iso_demo" ? `3px solid ${config.colorAcento}` : "3px solid transparent",
              color: seccion === "iso_demo" ? config.colorAcento : THEME.white,
              display: "flex",
              alignItems: "center",
              gap: "12px",
              fontSize: "14px",
              fontWeight: seccion === "iso_demo" ? 600 : 400
            }}
          >
            <span style={{ fontSize: "20px" }}>🎯</span>
            {!sidebarCollapsed && (
              <>
                <span style={{ flex: 1 }}>Políticas ISO 9001 Demo</span>
                <span style={{
                  backgroundColor: config.colorAcento,
                  color: config.colorPrimario,
                  padding: "2px 8px",
  ...
  const renderTopbar = () => {
    const seccionNombres = {
      ...
      iso: config.modulos?.iso?.nombre || "Políticas ISO 9001",
      iso_demo: config.modulos?.iso_demo?.nombre || "Políticas ISO 9001 Demo",
      oea: config.modulos?.oea?.nombre || "Requisitos OEA",
      procedimientos: config.modulos?.procedimientos?.nombre || "Procedimientos",
      formatos: config.modulos?.formatos?.nombre || "Formatos y Documentos",
      manuales: config.modulos?.manuales?.nombre || "Manuales",
      comunicados: config.modulos?.comunicados?.nombre || "Comunicados",
  ...
  // Actualizar formEmpresa cuando cambia config o sección
  useEffect(() => {
    if (seccion === "config") {
      setFormEmpresa({
        nombreEmpresa: config.nombreEmpresa,
      ...
      <div style={{ padding: "32px" }}>
        ...
        <div style={{
          ...
          {["empresa", "apariencia", "modulos", "usuarios", "datos"].map(tab => (
            <button
              key={tab}
              onClick={() => setTabActivoConfig(tab)}
              style={{
                padding: "12px 24px",
                border: "none",
                borderBottom: tabActivoConfig === tab ? `3px solid ${config.colorAcento}` : "3px solid transparent",
                background: "transparent",
                color: tabActivoConfig === tab ? THEME.textDark : THEME.textLight,
                cursor: "pointer",
                fontSize: "14px",
                fontWeight: tabActivoConfig === tab ? 600 : 400,
